...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const { dept_name, description, head_emp_id, phone, org_chart_url, sop_url, rules_url, sub_dept, capacity } = body;
    ...
    const [result] = await pool.query(
      'UPDATE tbl_departments SET dept_name = ?, description = ?, head_emp_id = ?, phone = ?, org_chart_url = ?, sop_url = ?, rules_url = ?, sub_dept = ?, capacity = ? WHERE dept_id = ?',
      [dept_name, description || null, head_emp_id || null, phone || null, org_chart_url || null, sop_url || null, rules_url || null, sub_dept || null, capacity || 0, dept_id]
    );
